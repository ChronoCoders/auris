...
async fn auth_login(
    ...
    let user = auris_db::get_user_by_username(&state.pool, &req.username)
        .await
...
async fn auth_register(
    ...
    let user_count = auris_db::get_user_count(&state.pool)
        .await
        .map_err(|e| ApiError::Internal(e.to_string()))?;
    if user_count > 0 {
    ...
    let user_id = auris_db::insert_user(&state.pool, &req.username, &hash)
        .await
        .map_err(|e| {
                ...
                match msg {
                    Message::Text(t) => {
                        ...
                        match cmd.r#type.as_str() {
                            "start_bot" => {
                                let _ = auris_db::set_bot_state(&state.pool, "running", "true").await;
                            }
                            "stop_bot" => {
                                let _ = auris_db::set_bot_state(&state.pool, "running", "false").await;
                            }
                            _ => {}
                        }
                    }
...
async fn bot_start(State(state): State<AppState>) -> Result<Json<serde_json::Value>, ApiError> {
    auris_db::set_bot_state(&state.pool, "running", "true")
        .await
        .map_err(|e| ApiError::Internal(e.to_string()))?;
    let block = auris_db::get_bot_state(&state.pool, "last_block")
        .await
        .map_err(|e| ApiError::Internal(e.to_string()))?
...
async fn bot_stop(State(state): State<AppState>) -> Result<Json<serde_json::Value>, ApiError> {
    auris_db::set_bot_state(&state.pool, "running", "false")
        .await
        .map_err(|e| ApiError::Internal(e.to_string()))?;
...
async fn bot_status(State(state): State<AppState>) -> Result<Json<serde_json::Value>, ApiError> {
    let running = auris_db::get_bot_state(&state.pool, "running")
        .await
        .map_err(|e| ApiError::Internal(e.to_string()))?
    ...
    let current_block = auris_db::get_bot_state(&state.pool, "last_block")
        .await
        .map_err(|e| ApiError::Internal(e.to_string()))?
        .and_then(|v| v.parse::<u64>().ok())
        .unwrap_or(0);

    let pools_monitored = auris_db::get_bot_state(&state.pool, "pools_monitored")
        .await
        .map_err(|e| ApiError::Internal(e.to_string()))?
        .and_then(|v| v.parse::<u64>().ok())
        .unwrap_or(0);
...
async fn stats_pnl(State(state): State<AppState>) -> Result<Json<serde_json::Value>, ApiError> {
    let pnl = auris_db::get_pnl_summary(&state.pool)
        .await
        .map_err(|e| ApiError::Internal(e.to_string()))?;
...
async fn stats_gas(State(state): State<AppState>) -> Result<Json<serde_json::Value>, ApiError> {
    ...
    let min_profit_gate_usd = auris_db::get_bot_state(&state.pool, "min_profit_usd")
        .await
        .map_err(|e| ApiError::Internal(e.to_string()))?
...
async fn trades_list(
    ...
    let total = auris_db::get_trade_count(&state.pool)
        .await
        .map_err(|e| ApiError::Internal(e.to_string()))?;
...
async fn push_ws_updates(
    ...
    let gate_usd = auris_db::get_bot_state(&state.pool, "min_profit_usd")
        .await
        .map_err(|e| ApiError::Internal(e.to_string()))?
        .and_then(|v| v.parse::<f64>().ok())
        .unwrap_or(0.0);
    ...
    let (scanned,): (i64,) = sqlx::query_as("SELECT COUNT(*) FROM opportunities")
        ...
        .await
        .map_err(|e| ApiError::Internal(e.to_string()))?;
    let executed = auris_db::get_trade_count(&state.pool)
        .await
        .map_err(|e| ApiError::Internal(e.to_string()))?;
