...
//   forge test --fork-url <erigon_node_url> --match-path test/AurisArb.t.sol
//
...
contract AurisArbTest is Test {
    AurisArb arb;
    address   owner;

    function setUp() public {
        owner = address(this);
        arb = new AurisArb(AAVE_PROVIDER);
    }
    ...
    function test_successful_arb() public {
        ...
        // Fund the AurisArb contract with WETH so it can repay the loan + profit
        ...
        AurisArb.ArbParams memory params = AurisArb.ArbParams({
            tokenIn:   WETH,
            tokenOut:  USDC,
            poolA:     UNI_USDC_WETH,
    ...
    function test_direct_execute_operation_reverts() public {
        bytes memory encoded = abi.encode(AurisArb.ArbParams({
            tokenIn:   WETH,
            tokenOut:  USDC,
            poolA:     UNI_USDC_WETH,
            poolB:     SUSHI_USDC_WETH,
