...
async fn run_gas_sampler_once(
    ...
    while let Some(block) = stream.next().await {
        ...
        auris_db::insert_gas_sample(pool, block_number, base_gwei, priority_gwei).await?;
        auris_db::set_bot_state(pool, "last_block", &block_number.to_string()).await?;
