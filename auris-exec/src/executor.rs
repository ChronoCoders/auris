...
use sqlx::SqlitePool;
use tracing::info;
use auris_types::{ArbitrageOpportunity, TradeResult, TradeStatus};

use crate::{
    flashbots::FlashbotsClient, inclusion::wait_for_inclusion, load_signer_from_file,
    tx_builder::build_execute_arb_calldata, ExecError,
};
...
impl ArbitrageExecutor {
    ...
    pub async fn execute(
        ...
        if self.paper_trade {
            ...
            let trade_id = auris_db::insert_trade(&self.db, &trade)
                .await
            ...
            let included = wait_for_inclusion(&self.provider, tx_hash, target_block).await?;
    ...
    /// Build and sign an EIP-1559 transaction targeting the AurisArb contract.
    async fn build_signed_tx(&self, input: Bytes) -> Result<Bytes, ExecError> {
        ...
        let block = self
            ...
            .await
            .map_err(|e| ExecError::ProviderError(e.to_string()))?
            .ok_or_else(|| ExecError::ProviderError("latest block not found".to_string()))?;
