...
mod tests {
    ...
    fn calldata_encodes_and_decodes_roundtrip() {
        ...
        let cd =
            build_execute_arb_calldata(token_in, token_out, pool_a, pool_b, amount_in, min_profit);

        let decoded = executeArbCall::abi_decode(&cd, true).expect("must decode successfully");
