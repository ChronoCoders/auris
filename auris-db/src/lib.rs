...
use sqlx::SqlitePool;
use thiserror::Error;
use auris_types::{ArbitrageOpportunity, OpportunityRecord, TradeResult, TradeStatus};
...
mod tests {
    ...
    use sqlx::SqlitePool;
    use auris_types::{TradeResult, TradeStatus};
    ...
    async fn opportunities_insert(pool: SqlitePool) -> sqlx::Result<()> {
        ...
        let opp = auris_types::ArbitrageOpportunity {
            pool_a: Address::ZERO,
            pool_b: Address::ZERO,
    ...
    async fn get_opportunities_for_backtest_returns_rows(pool: SqlitePool) -> sqlx::Result<()> {
        ...
        let base_opp = auris_types::ArbitrageOpportunity {
            pool_a: Address::ZERO,
            pool_b: Address::ZERO,
            token_in: Address::ZERO,
            token_out: Address::ZERO,
    ...
    async fn backtest_end_to_end(pool: SqlitePool) -> sqlx::Result<()> {
        ...
        let report = auris_sim::backtest::run_backtest(&rows, 5.0);
        ...
        let report_raised = auris_sim::backtest::run_backtest(&rows, 12.0);
