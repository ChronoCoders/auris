...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let config = auris_config::load(&config_path)?;

    let db_url = sqlite_url(&config.database.path);
    let pool = sqlx::SqlitePool::connect(&db_url).await?;
    auris_db::run_migrations(&pool).await?;

    let signer = auris_exec::load_signer_from_file(&config.keys.signer_key_path)?;
    let wallet_address = signer.address();

    let pools = resolve_pool_addresses(&config)?;
    let pool_metadata = fetch_pool_metadata(&config.node.ws_url, pools.clone()).await?;
    auris_db::set_bot_state(&pool, "pools_monitored", &pools.len().to_string()).await?;
    auris_db::set_bot_state(
        &pool,
        "min_profit_usd",
        &config.bot.min_profit_usd.to_string(),
    )
    .await?;
    auris_db::set_bot_state(&pool, "max_gas_gwei", &config.bot.max_gas_gwei.to_string()).await?;

    let pool_store = std::sync::Arc::new(Mutex::new(auris_core::sync::PoolStore::new()));
    {
    ...
    let gas_oracle = std::sync::Arc::new(Mutex::new(auris_gas::GasOracle::new()));

    {
        ...
        tokio::spawn(async move {
            if let Err(e) = auris_gas::run_gas_sampler(&ws_url, pool, gas_oracle).await {
...

    {
        ...
        tokio::spawn(async move {
            if let Err(e) = auris_core::sync_subscriber::run_sync_subscription(
    ...
    let executor = auris_exec::ArbitrageExecutor::new(
        &config.node.ws_url,
    ...
    {
        let scanner_cfg = auris_core::scanner::ScannerConfig {
        ...
        tokio::spawn(async move {
            if let Err(e) = auris_core::scanner::run_scanner(scanner_cfg, updates_rx).await {
                eprintln!("scanner fatal: {e}");
            }
        });
    }

    let state = auris_api::AppState {
        pool: pool.clone(),
    ...
    let app = auris_api::router(state);
...
fn resolve_pool_addresses(
    ...
    Ok(auris_core::sync_subscriber::default_test_pools().to_vec())
}

async fn fetch_pool_metadata(
    ws_url: &str,
    pool_addresses: Vec<Address>,
) -> Result<Vec<auris_types::Pool>, Box<dyn std::error::Error>> {
