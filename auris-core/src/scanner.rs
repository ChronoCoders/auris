...
use auris_exec::ArbitrageExecutor;
use auris_gas::GasOracle;
use auris_sim::evaluate;
use auris_types::{ArbitrageOpportunity, ProfitDecision};
        ...
        for mut opp in opportunities {
            ...
            let trade_id: Option<i64> = if matches!(decision, ProfitDecision::Go { .. }) {
                if let Some(ref exec) = executor {
                    ...
                    match exec.execute(&opp, U256::from(1u32), current_block).await {
                        Ok(id) => {
            ...
            auris_db::insert_opportunity(&db, &opp, decision_str, reason.as_deref(), trade_id)
                .await?;
        }
    }
...
fn build_opportunity(
    pool_a: &auris_types::Pool,
    pool_b: &auris_types::Pool,
    token_out: Address,
    amount_in: U256,
    eth_price_usd: f64,
