...
fn apply_env_overrides(config: &mut Config) -> Result<(), ConfigError> {
    env_override(&mut config.node.ws_url, "AURIS_NODE_WS_URL", |v| {
        Ok(v.to_string())
    })?;
    env_override(
        &mut config.flashbots.relay_url,
        "AURIS_FLASHBOTS_RELAY_URL",
        |v| Ok(v.to_string()),
    )?;
    env_override(
        &mut config.bot.min_profit_usd,
        "AURIS_BOT_MIN_PROFIT_USD",
        |v| v.parse::<f64>().map_err(|_| ()),
    )?;
    env_override(
        &mut config.bot.max_gas_gwei,
        "AURIS_BOT_MAX_GAS_GWEI",
        |v| v.parse::<u64>().map_err(|_| ()),
    )?;
    env_override(&mut config.bot.paper_trade, "AURIS_BOT_PAPER_TRADE", |v| {
        v.parse::<bool>().map_err(|_| ())
    })?;
    env_override(
        &mut config.pools.seed_pairs,
        "AURIS_POOLS_SEED_PAIRS",
        |v| {
            Ok(v.split(',')
    ...
    env_override(
        ...
        "AURIS_SERVER_WEB_UI_PATH",
        |v| Ok(PathBuf::from(v)),
    )?;
    env_override(&mut config.database.path, "AURIS_DATABASE_PATH", |v| {
        Ok(PathBuf::from(v))
    })?;
    env_override(&mut config.auth.jwt_secret, "AURIS_AUTH_JWT_SECRET", |v| {
        Ok(v.to_string())
    })?;
    env_override(
        &mut config.auth.jwt_expiry_seconds,
        "AURIS_AUTH_JWT_EXPIRY_SECONDS",
        |v| v.parse::<u64>().map_err(|_| ()),
    )?;
    env_override(
